...
export function createAuthRouter(): Router {
  ...
  router.post('/register', (req: Request, res: Response) => {
    ...
    if (!/@ialibu\.edu\.pg$/i.test(String(email))) {
      return res.json({ success: false, message: 'Email must be a valid ialibu.edu.pg address' });
    }
    const exists = users.some(u => u.username === username || u.email.toLowerCase() === String(email).toLowerCase());
    if (exists) {
    ...
    const newUser: UserRec = {
      ...
      isActive: true,
      createdAt: new Date().toISOString(),
      permissions: userType === 'admin' ? ['all'] : ['students', 'attendance', 'grades', 'reports'],
      assignedClasses: [],
      assignedSubjects: [],
  ...
  router.post('/login', (req: Request, res: Response) => {
    ...
    if (user.password !== password) return res.json({ success: false, message: 'Invalid username or password' });

    user.lastLogin = new Date().toISOString();
    const token = `token_${user.id}_${Date.now()}`;

    return res.json({ success: true, message: 'Login successful', user: toApiUser(user), token });
  });
...
export function createUsersRouter(): Router {
  ...
  router.post('/', (req: Request, res: Response) => {
    const { username, email, password, firstName, lastName, userType, assignedClasses, assignedSubjects, allowCrossClass } = req.body || {};
    if (!username || !email || !password || !firstName || !lastName || !userType) {
      return res.json({ success: false, message: 'Missing required fields' });
    }
    if (!/@ialibu\.edu\.pg$/i.test(String(email))) {
    ...
    const exists = users.some(u => u.username === username || u.email.toLowerCase() === String(email).toLowerCase());
    if (exists) return res.json({ success: false, message: 'Username or email already exists' });
  ...
  router.put('/:id', (req: Request, res: Response) => {
    ...
    users[idx] = {
      ...
      permissions: Array.isArray(updates.permissions) ? updates.permissions : u.permissions,
      assignedClasses: Array.isArray(updates.assignedClasses) ? updates.assignedClasses : u.assignedClasses,
      assignedSubjects: Array.isArray(updates.assignedSubjects) ? updates.assignedSubjects : u.assignedSubjects,
  ...
  router.delete('/:id', (req: Request, res: Response) => {
    ...
    if (u.username === 'admin') return res.json({ success: false, message: 'Cannot delete the admin user' });
    u.isActive = false;
